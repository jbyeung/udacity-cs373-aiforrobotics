...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #x_t+1 -> expand(), take() new omega included expanded section.  calculate A, B, C
    #omega = omega' - A(T)*B(-1)*A
    #xi = xi' - A(T)*B(-1)*C
    #iteratively expand, run slam, take, recalculate new omega and Xi

    #initialize
    dim = 2 * (1 + num_landmarks)
    index = 0    # index of last x we have inserted into Omega

    # make the constraint information matrix and vector for initial position and map
    Omega = matrix()
    Omega.zero(dim, dim)
    Omega.value[0][0] = 1.0
    Omega.value[1][1] = 1.0
    ...
    #for take functions later, index the x,y of all poses,res in Omega

    # iteratively expand each item, process motion/localization, take new matrices
    for k in range(len(data)):

        #expand new term index+1
        index_expand = []
        index_expand.append(0)
        index_expand.append(1)
        for c in range(4, 4 + (2*num_landmarks)):
            index_expand.append(c)


        Omega = Omega.expand(dim+2, dim+2, index_expand, index_expand)
        Xi = Xi.expand(dim+2, 1, index_expand, [0])

        #run SLAM to add new pose, update omega,xi
        ...
        #compute A, B, C, Omega', Xi'
        index_map = []
        for c in range(4, 4+(2*num_landmarks)):
            index_map.append(c)
        index_prime = []
        for c in range(2, 4+(2*num_landmarks)):
            index_prime.append(c)

        B = Omega.take([0, 1], [0, 1])
        C = Xi.take([0, 1], [0])
        A = Omega.take([0, 1], index_prime)
        OmegaP = Omega.take(index_prime, index_prime)
        XiP = Xi.take(index_prime, [0])

        #compute new Omega, Xi
