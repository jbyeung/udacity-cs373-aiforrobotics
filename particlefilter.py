...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        d = motion[1] * (1+random.gauss(0, self.distance_noise))
        alpha = motion[0] * (1+random.gauss(0, self.steering_noise))
    ...
    def sense(self, flag=1): #do not change the name of this function
        ...
        if not flag:
            for M in landmarks:
                Z.append((atan2(M[0]-self.y, M[1]-self.x) - self.orientation) % (2.*pi))
        else:
            #simulate self.bearing_noise
            for M in landmarks:
                noise = random.gauss(0, self.bearing_noise)
                Z.append((atan2(M[0]*(1+noise)-self.y, M[1]*(1+noise)-self.x) - self.orientation) % (2*pi))
